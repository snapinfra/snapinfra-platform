...
export function getModulePrompts(project: any) {
  ...
  const generateTableSQL = (table: any) => {
    ...
    // 🔥 FIX: Filter out fields that we generate automatically
    const autoGeneratedFields = ['id', 'created_at', 'updated_at', 'deleted_at'];
    const fieldsWithoutAutoGenerated = table.fields.filter((field: any) =>
      !autoGeneratedFields.includes(field.name.toLowerCase())
    );

    const fields = fieldsWithoutAutoGenerated.map((field: any) => {
    ...
    // Add foreign key constraints (also filter out auto-generated fields)
    const foreignKeys = fieldsWithoutAutoGenerated
...
${fieldsWithoutAutoGenerated.filter((f: any) => f.unique || f.references).map((f: any) =>
